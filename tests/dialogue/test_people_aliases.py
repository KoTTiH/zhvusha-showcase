...
def test_extract_explicit_username_alias_candidate() -> None:
    ...
    candidates = extract_people_alias_candidates(
        "@demo_recipient_1 это Тоша",
        source_message_id="tg:10",
    )

    assert len(candidates) == 1
    assert candidates[0].alias == "Тоша"
    assert candidates[0].executable_chat_id == "@demo_recipient_1"
...
def test_alias_lookup_requires_confirmation_and_never_executes(tmp_path: Path) -> None:
    ...
    assert result.suggested_recipient == "@demo_recipient_1"
...
def test_alias_lookup_rejects_low_confidence_candidate(tmp_path: Path) -> None:
    ...
    store.append(
        ...
        candidate=PeopleAliasCandidate(
            ...
            source_text="@demo_recipient_1 возможно Тоша",
            confidence=0.4,
        ),
    )
    ...
    assert result.can_execute is False
    assert result.candidates[0].executable_chat_id == "@demo_recipient_1"
...
def test_alias_lookup_rejects_stale_candidate(tmp_path: Path) -> None:
    ...
    store.append(
        ...
        candidate=PeopleAliasCandidate(
            alias="Тоша",
            executable_chat_id="@demo_recipient_1",
            source_text="@demo_recipient_1 это Тоша",
            confidence=0.95,
...
def test_render_people_alias_lookup_status_hides_raw_source_text(
    ...
    store.append(
        ...
        candidate=PeopleAliasCandidate(
            alias="Тоша",
            executable_chat_id="@demo_recipient_1",
            source_text="@demo_recipient_1 это Тоша и секретный контекст",
            confidence=0.95,
        ),
    )
    ...
    assert "suggested_recipient: @demo_recipient_1" in status
