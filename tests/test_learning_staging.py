...
def test_staging_writer_includes_original_claim_for_correction(tmp_path: Path) -> None:
    ...
    content = target.read_text(encoding="utf-8")
    assert (
        "**Original claim:** В тестовом примере пользователь изучает Python по утрам."
        in content
    )
...
def test_staging_writer_semantic_dedup_catches_distant_duplicate(
    ...
    # Проверяем использование семантического сравнения, без загрузки ML-модели.
    with patch(
        "src.embeddings.EmbeddingService.embed_batch",
        return_value=[[1.0, 0.0], [1.0, 0.0], [0.0, 1.0]],
    ) as embed_batch:
        second = writer.append(_make_signal(statement="долгое правило"), episode_id=2)
    embed_batch.assert_called_once_with(["долгое правило", "долгое правило", filler])
