...
def _isolate_dream_check(monkeypatch: pytest.MonkeyPatch) -> None:
    """Этот набор проверяет enrichment, без отдельного фонового LLM-вызова."""
    monkeypatch.setattr(ChatResponseSkill, "_background_dream_check", AsyncMock())
...
def _settings(tmp_path: str = "test_ws") -> SimpleNamespace:
    return SimpleNamespace(
        ...
        public_info_about_nikita="Alexey is a developer.",
        admin_user_id=12345,
    )
...
def _sample_result(**overrides: object) -> EnrichmentResult:
    base: dict[str, object] = {
        ...
        "reasoning": "Alexey asking about his code.",
...
def _weak_learning_signal(**overrides: object) -> LearningSignal:
    base: dict[str, object] = {
        "type": "preference",
        "statement": "кажется Алексей предпочитает краткие ответы",
        "scope": "preferences",
        "confidence": 0.6,
...
def _correction_learning_signal(**overrides: object) -> LearningSignal:
    base: dict[str, object] = {
        "type": "correction",
        "statement": "В тестовом примере пользователь изучает Python по вечерам.",
        "scope": "personal_facts",
        "confidence": 0.92,
        "apply_immediately": True,
        "original_claim": "В тестовом примере пользователь изучает Python по утрам.",
    }
    base.update(overrides)
    return LearningSignal(**base)  # type: ignore[arg-type]
...
def _mock_enricher(
    *,
    return_value: EnrichmentResult | object | None = _SENTINEL,
    side_effect: Exception | None = None,
) -> AsyncMock:
...
async def test_try_resolve_learning_correction_triggers_rejection_handler(
    ...
    engine.handle_explicit_rejection.assert_awaited_once_with(
        rejected_conclusion="В тестовом примере пользователь изучает Python по утрам.",
        nikita_correction="В тестовом примере пользователь изучает Python по вечерам.",
    )
