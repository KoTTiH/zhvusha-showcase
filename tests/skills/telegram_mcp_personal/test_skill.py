...
def _clarification_summary(result: SkillResult) -> str:
    """Проверяем передачу уточнения оркестратору, который формирует ответ."""
    assert result.success is True
    assert result.response == ""
    assert result.metadata["requires_user_input"] is True
    assert result.metadata["requires_zhvusha_response"] is True
    decision = result.metadata["pending_decision"]
    assert decision["kind"] == "missing_required_input"
    assert decision["required_consent"] is False
...
async def test_natural_language_send_intent_routes_above_chat_fallback_score() -> None:
    ...
    message = "Через личный Telegram отправь Алексею: привет"
    ...
    assert plan.human_summary == (
        "Отправить личное Telegram сообщение в @demo_operator: «привет»"
    )
    ...
    assert '"chat_id": "@demo_operator"' in pack.user_request
...
async def test_codex_author_marker_does_not_skip_telegram_mcp_classifier() -> None:
    ...
    assert score == 0.91
    assert classifier.calls == ["скинь Демо ок"]
    assert runtime.created == []
...
async def test_natural_language_send_intent_asks_short_clarification_then_approval() -> (
    ...
    clarification = await service.dispatch(
        "Напиши Алексею через личный Telegram",
        _ctx(),
        [skill],
    )
    ...
    assert (
        _clarification_summary(clarification.result) == "Что написать в @demo_operator?"
    )
...
async def test_pending_clarification_meta_question_is_not_used_as_message_text() -> (
    ...
    clarification = await service.dispatch("Напишешь Демо?", _ctx(), [skill])

    assert clarification.result is not None
    assert _clarification_summary(clarification.result) == (
        "Не хватает @username/id для Демо и текста сообщения."
    )
    assert clarification.result.metadata["requires_zhvusha_response"] is True
    assert "в демо" not in _clarification_summary(clarification.result).lower()

    meta_reply = await service.dispatch('Почему "в"?', _ctx(), [skill])

    assert meta_reply.result is not None
    assert _clarification_summary(meta_reply.result) == (
        "Не хватает @username/id для Демо и текста сообщения."
    )
    assert "Нужно решение" not in _clarification_summary(meta_reply.result)
    assert 'Почему "в"?' not in _clarification_summary(meta_reply.result)
    assert runtime.created == []
...
async def test_pending_human_recipient_replies_do_not_reset_to_generic_question() -> (
    ...
    first = await service.dispatch("Напишешь Демо?", _ctx(), [skill])
    second = await service.dispatch("Демо", _ctx(), [skill])
    third = await service.dispatch("Демо написать", _ctx(), [skill])

    assert first.result is not None
    assert second.result is not None
    assert third.result is not None
    expected = "Не хватает @username/id для Демо и текста сообщения."
    assert _clarification_summary(first.result) == expected
    assert _clarification_summary(second.result) == expected
    assert _clarification_summary(third.result) == expected
    assert runtime.created == []
...
async def test_pending_recipient_correction_drops_previous_creator_chat_id() -> None:
    ...
    correction = await service.dispatch("Не мне, а Демо", _ctx(), [skill])
    ...
    assert first.result is not None
    assert _clarification_summary(first.result) == "Что написать в @demo_operator?"
    assert correction.result is not None
    assert username_meta.result is not None
    assert frustration.result is not None
    expected = "Не хватает @username/id для Демо и текста сообщения."
    assert _clarification_summary(correction.result) == expected
    assert _clarification_summary(username_meta.result) == expected
    assert _clarification_summary(frustration.result) == expected
    assert "@demo_operator" not in _clarification_summary(correction.result)
    assert "@demo_operator" not in _clarification_summary(username_meta.result)
    assert "@demo_operator" not in _clarification_summary(frustration.result)
    assert runtime.created == []
...
async def test_pronoun_send_followup_uses_recent_recipient_hint_and_draft_not_creator() -> (
    ...
    classifier = _IntentClassifier(
        TelegramMCPActionIntent(
            ...
            confidence=0.9,
            recipient_hint="Демо",
            message="Демо, предлагаю обсудить учебный проект завтра.",
        )
    )
    skill = TelegramMCPPersonalSkill(
        admin_user_id=12345,
    ...
    recent_messages = "\n".join(
        (
            "Собеседник: Не мне, а Демо",
            "Собеседник: Это мой юсернейм",
            (
                "Жвуша: Адресат — Демо. Указанный username принадлежит тебе; "
                "нужен username адресата.\n\n"
                "Написала бы так:\n\n"
                "Демо, предлагаю обсудить учебный проект завтра."
            ),
        )
    )
    ...
    assert _clarification_summary(outcome.result) == "Не хватает @username/id для Демо."
    assert "@demo_operator" not in _clarification_summary(outcome.result)
    assert runtime.created == []
    assert "Собеседник: Не мне, а Демо" in classifier.calls[-1]
...
async def test_pronoun_send_followup_prefers_dialogue_state_over_stale_history() -> (
        ...
        _ctx(
            metadata={
                ...
                "dialogue_context": (
                    ...
                    "draft_message: Демо, предлагаю обсудить учебный проект завтра.\n"
                    "missing_fields: chat_id"
                ),
                "recent_decision_messages": (
                    "Собеседник: Не мне, а Демо\nЖвуша: Не хватает @username/id."
                ),
                "recent_messages": "Жвуша: Что написать в @demo_operator?",
            }
        ),
        [skill],
    )
    ...
    assert "recipient_hint: Демо" in prompt
    assert "<RECENT_DECISION_CONTEXT>" in prompt
    assert "Жвуша: Что написать в @demo_operator?" not in prompt
...
async def test_alias_lookup_proposes_known_username_without_execution(
    ...
    classifier = _IntentClassifier(
        TelegramMCPActionIntent(
            ...
            confidence=0.92,
            recipient_hint="Демо",
            message="Демо, обсудим учебный проект?",
        )
    )
    skill = TelegramMCPPersonalSkill(
        admin_user_id=12345,
    ...
    outcome = await service.dispatch("Пиши Демо", _ctx(), [skill])

    assert outcome.result is not None
    assert "Демо" in _clarification_summary(outcome.result)
    assert "@demo_recipient_1" in _clarification_summary(outcome.result)
    assert runtime.created == []

    metadata = outcome.result.metadata
    assert metadata["missing_fields"] == ["chat_id"]
    assert metadata["suggested_telegram_recipient"] == "@demo_recipient_1"
    ...
    assert patch["recipient_hint"] == "Демо"
...
async def test_alias_lookup_confirmation_promotes_suggestion_to_approval_plan(
    ...
    suggestion = await service.dispatch("Пиши Демо", _ctx(), [skill])
    assert suggestion.result is not None
    assert suggestion.result.metadata["requires_recipient_confirmation"] is True
    assert (
        suggestion.result.metadata["suggested_telegram_recipient"]
        == "@demo_recipient_1"
    )
    ...
    assert approval.result is not None
    assert "Отправить личное Telegram сообщение в @demo_recipient_1" in (
        approval.result.response
    )
    assert "Демо, обсудим учебный проект?" in approval.result.response
    assert runtime.created == []
    ...
    assert created["profile"] == TELEGRAM_MCP_PERSONAL_ACTIONS
    pack = created["context_pack"]
    assert "@demo_recipient_1" in pack.user_request
    assert "Демо, обсудим учебный проект?" in pack.user_request
...
async def test_alias_lookup_rejection_suppresses_same_suggestion_then_accepts_correction(
    ...
    correction = await service.dispatch("@demo_recipient_updated", _ctx(), [skill])

    assert suggestion.result is not None
    assert rejection.result is not None
    assert correction.result is not None
    assert _clarification_summary(rejection.result) == (
        "Ок, не @demo_recipient_1. Пришли @username/id для Демо."
    )
    ...
    assert "Отправить личное Telegram сообщение в @demo_recipient_updated" in (
...
async def test_pending_creative_reply_uses_classifier_draft_not_literal_instruction() -> (
    ...
    classifier = _IntentClassifier(
        ...
        TelegramMCPActionIntent(
            ...
            confidence=0.91,
            chat_id="@demo_operator",
            message="Предлагаю начать с проверки учебного примера.",
        ),
    )
    skill = TelegramMCPPersonalSkill(
        admin_user_id=12345,
    ...
    assert approval.handled is True
    assert approval.result is not None
    assert "Предлагаю начать с проверки" in approval.result.response
    ...
    pack = runtime.created[0]["context_pack"]
    assert "Предлагаю начать с проверки" in pack.user_request
...
async def test_followup_after_rejected_send_keeps_previous_recipient_context() -> None:
    ...
    classifier = _IntentClassifier(
        ...
        TelegramMCPActionIntent(
            ...
            confidence=0.91,
            chat_id="@demo_recipient_1",
            message="Проверь учебный пример",
        ),
        TelegramMCPActionIntent(
            action="send_message",
            confidence=0.92,
            chat_id="@demo_recipient_1",
            message="Можешь проверить обновлённый учебный пример?",
        ),
    )
    skill = TelegramMCPPersonalSkill(
        admin_user_id=12345,
    ...
    clarification = await service.dispatch(
        "@demo_recipient_1 это Демо, черкани ему в личку",
        _ctx(),
        [skill],
    )
    assert clarification.result is not None
    assert (
        _clarification_summary(clarification.result)
        == "Что написать в @demo_recipient_1?"
    )

    rejected_approval = await service.dispatch(
        "Проверь учебный пример", _ctx(), [skill]
    )
    ...
    assert "@demo_operator" not in followup_approval.result.response
    assert "previous_telegram_chat_id: @demo_recipient_1" in classifier.calls[-1]
    ...
    assert executed.result.response == "отправила."
    pack = runtime.created[0]["context_pack"]
    assert '"chat_id": "@demo_recipient_1"' in pack.user_request
    assert "обновлённый учебный пример" in pack.user_request
...
async def test_pending_approval_text_feedback_rewrites_draft_instead_of_yes_no_loop() -> (
    ...
    classifier = _IntentClassifier(
        TelegramMCPActionIntent(
            ...
            confidence=0.92,
            chat_id="@demo_recipient_1",
            message=("Демо-контакт, предлагаю обсудить учебный проект завтра."),
        ),
        TelegramMCPActionIntent(
            action="send_message",
            confidence=0.94,
            chat_id="@demo_recipient_1",
            message="Обсудим учебный проект завтра вечером?",
        ),
    )
    skill = TelegramMCPPersonalSkill(
        admin_user_id=12345,
    ...
    approval = await service.dispatch(
        "напиши @demo_recipient_1 про обсуждение учебного проекта завтра",
        _ctx(),
        [skill],
    )
    assert approval.result is not None
    assert "предлагаю обсудить" in approval.result.response

    revision = await service.dispatch(
        "слишком официально, перепиши короче", _ctx(), [skill]
    )

    assert revision.result is not None
    assert "Ответь явно" not in revision.result.response
    assert "Обсудим учебный проект завтра вечером?" in revision.result.response
    assert "предлагаю обсудить" not in revision.result.response
    assert "Фидбек Алексея" in classifier.calls[-1]
    assert runtime.created == []
    ...
    pack = runtime.created[0]["context_pack"]
    assert '"chat_id": "@demo_recipient_1"' in pack.user_request
    assert "Обсудим учебный проект завтра вечером?" in pack.user_request
    assert "предлагаю обсудить" not in pack.user_request
...
async def test_named_recipient_followup_keeps_recent_real_chat_id_context() -> None:
    ...
    classifier = _IntentClassifier(
        TelegramMCPActionIntent(
            ...
            confidence=0.9,
            chat_id="@demo_recipient_1",
            message="Когда обсудим учебный проект?",
        ),
        TelegramMCPActionIntent(
            action="send_message",
            confidence=0.91,
            chat_id="@demo_recipient_1",
            message="Пришли, пожалуйста, заметки по учебному проекту.",
        ),
    )
    skill = TelegramMCPPersonalSkill(
        admin_user_id=12345,
    ...
    await service.dispatch(
        "напиши @demo_recipient_1 и спроси, когда обсудим учебный проект",
        _ctx(),
        [skill],
    )
    await service.dispatch("да", _ctx(), [skill])
    second = await service.dispatch(
        "напиши демо, чтобы прислал заметки по учебному проекту",
        _ctx(),
        [skill],
    )
    ...
    assert second.result is not None
    assert (
        "Отправить личное Telegram сообщение в @demo_recipient_1"
        in second.result.response
    )
    assert "previous_telegram_chat_id: @demo_recipient_1" in classifier.calls[-1]
    assert "Человеческое имя без @username" in classifier.calls[-1]
    assert "Демо" not in second.result.response
