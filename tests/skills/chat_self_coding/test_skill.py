    ...
    async def test_entry_reopens_existing_session_without_losing_context(self) -> None:
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                is_open=False,
                recent_messages=("Алексей: обсудили host ops",),
        ...
        assert state.is_open is True
        assert state.recent_messages == ("Алексей: обсудили host ops",)
    ...
    async def test_clear_resets_context_but_keeps_code_mode_open(self) -> None:
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                recent_messages=("Алексей: старый контекст",),
    ...
    async def test_compact_resets_readonly_codex_thread_but_preserves_summary(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                recent_messages=("Алексей: дорого", "Жвуша: причина в ephemeral"),
    ...
    async def test_exit_closes_mode_but_keeps_session(self) -> None:
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                user_id=1,
                stage=Stage.IDLE,
                recent_messages=("Алексей: важный контекст",),
            )
        )
        ...
        assert state.recent_messages == ("Алексей: важный контекст",)
    ...
    async def test_complete_trigger_archives_and_clears_session(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                stage=Stage.IDLE,
                is_open=False,
                recent_messages=("Алексей: старый хвост",),
            )
        )
        ...
        assert "Алексей: старый хвост" in archive_text
...
class TestIntentRouting:
    ...
    async def test_implicit_entry_from_chat_followup_starts_spec_creation(
        ...
        assert "Текущая команда Алексея" in sent_message
        state = await w.state_store.load(1)
        assert state is not None
        assert state.is_open is True
        assert state.stage == Stage.PENDING_APPROVAL
    ...
    async def test_approve_can_run_implementation_through_agent_runtime(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                stage=Stage.PENDING_APPROVAL,
                active_spec_slug="my-spec",
                recent_messages=("Алексей: оформи план", "Жвуша: план готов"),
            )
        )
    ...
    async def test_background_auto_retryable_failure_is_not_restarted(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                stage=Stage.PENDING_APPROVAL,
                active_spec_slug="my-spec",
                recent_messages=("Алексей: media-first unsafe",),
            )
        )
        bot = SimpleNamespace(send_message=AsyncMock())
    ...
    async def test_recovery_resume_passes_discussion_to_direct_implementation(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                active_spec_slug="my-spec",
                recent_messages=(
                    "Алексей: reviewer прав про media-first",
                    "Алексей: надо сначала публиковать текст",
                ),
            ).with_recovery(
                kind="run_spec",
                text="my-spec",
        ...
        assert "Алексей: надо сначала публиковать текст" in recent
        assert "Алексей: продолжай" in recent
    ...
    async def test_implementation_runner_receives_stable_code_task_context(
        ...
        async def implementation_runner(
            *,
            ...
            assert slug == "my-spec"
            assert "Алексей: reviewer прав" in recent_messages
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                stage=Stage.PENDING_APPROVAL,
                active_spec_slug="my-spec",
                recent_messages=("Алексей: reviewer прав",),
    ...
    async def test_explorer_prompt_contains_discussion_context_and_attachment_paths(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                stage=Stage.IDLE,
                recent_messages=(
                    "Алексей: хочу обсудить экран",
                    "Алексей прислал вложение для /код\n"
    ...
    async def test_create_after_discussion_forwards_context_to_ideation(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                stage=Stage.IDLE,
                recent_messages=(
                    "Алексей: хочу обсудить приветствие",
    ...
    async def test_idle_run_trigger_after_discussion_does_not_start_spec(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                stage=Stage.IDLE,
                recent_messages=(
                    "Алексей: давай обсудим проблему приветствий",
    ...
    async def test_create_after_pending_discussion_replaces_active_spec(
        ...
        await w.state_store.save(
            ChatSelfCodingState(
                ...
                recent_messages=(
                    "Алексей: старый план не учитывает guard",
        ...
        assert entries[0]["text"] == "Алексей: пересобери план"
    ...
    async def test_discussion_roundtrip_is_written_to_task_transcript(
        ...
        assert entries[0]["text"] == "Алексей: давай обсудим runtime"
