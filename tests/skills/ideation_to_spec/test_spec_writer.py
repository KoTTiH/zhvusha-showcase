...
def _draft_dict(**overrides: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
        ...
        "rationale": (
            "Алексей попросил новый weather skill; похожих skill'ов нет, "
...
class TestBuildSpecFromDraft:
    def test_validates_and_returns_spec_model(self) -> None:
        ...
        assert spec.tier == 1
        assert spec.rationale.startswith("Алексей попросил")
    ...
    def test_personality_anchor_contract_draft_becomes_tier3(self) -> None:
        ...
        draft = _draft_dict(
            ...
            rationale=(
                "Алексей просит общий contract личности, поэтому менять нужно "
