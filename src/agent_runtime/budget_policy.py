    ...
    def from_settings(cls, settings: Settings) -> AutonomyResourcePolicy:
        ...
        profiles = {
            ...
            BudgetJobKind.TIER3_REVIEW: _resource_profile(
                ...
                notes=("Tier 3 review requires Алексей, never autonomous approval.",),
            ),
        }
        return cls(profiles)
    ...
    def evaluate(
        ...
        if profile.requires_nikita:
            return AutonomyResourcePolicyDecision(
                ...
                status_lines=(
                    *status_lines,
                    "Tier 3 не запускается автономно: нужен Алексей.",
                ),
            )
    ...
    def from_settings(cls, settings: Settings) -> AgentBudgetPolicy:
        ...
        routes = {
            ...
            BudgetJobKind.TIER3_REVIEW: _tier_route(
                ...
                requires_nikita=True,
                notes=(
                    "Tier 3 review is a Алексей decision, not an auto budget decision.",
                ),
            ),
        }
        return cls(routes)
    ...
    def evaluate(
        ...
        if route.requires_nikita:
            return BudgetPolicyDecision(
                ...
                status_lines=(
                    *status_lines,
                    "Tier 3 review не запускается автономно: нужен Алексей.",
                ),
            )
