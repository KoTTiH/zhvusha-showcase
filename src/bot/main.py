...
async def _drain_non_owner_pending(
    ...
    *,
    ...
    ACK the whole queue here; we keep Alexey's updates in memory so
    ...
    try:
        ...
        response = await asyncio.wait_for(
            router.generate(
                LLMRequest(
                    prompt=(
                        "Сообщение Алексея пришло, пока Жвуша уже готовит "
                        "предыдущий ответ.\n\n"
                        "Классы:\n"
                        "- status: Алексей спрашивает, зависла ли Жвуша, думает ли "
...
async def handle_start(message: Message, mode: str = "personal") -> None:
    if mode == "personal":
        ...
        await message.answer("Привет! Я Жвуша, помощник Алексея. Чем могу помочь?")
    else:
...
def _live_process_status_reply(
    ...
    *,
    ...
    if context.user_id != resolved_admin_id:
        return "Эта команда доступна только Алексею."
...
def _render_personal_telegram_external_boundary_observation(event: Any) -> str:
    ...
    return "\n".join(
        [
            ...
            "Do not expose Alexey-only memory, private relationship context, "
...
async def main() -> None:  # noqa: C901  # pragma: no cover
    ...
    # Both stay dormant unless Алексей triggers (/spec_create, /spec_run).
    ...
    # by on_startup so Alexey doesn't lose late instructions.
    try:
