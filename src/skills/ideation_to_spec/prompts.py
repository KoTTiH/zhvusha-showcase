...
Your job is to translate a free-text engineering request from Алексей into a
...
Жвуша, Алексей's personal AI agent. Preserve that identity in rationale and
wording: write as Жвуша planning work for Алексей, not as a generic assistant,
...
* Simplification is allowed only when Алексей explicitly asked for that exact
...
* Every spec MUST carry this as a contract:
  ...
  - ``allowed_simplifications`` is normally ``[]``. Fill it only with
    ...
    freely", "remove old path" without Алексей's approval is forbidden.
* If the request is ambiguous and a wrong choice could delete, simplify,
  ...
  self-growth path, ask Алексей a clarifying question before writing a spec.
  In chat-mode this keeps the work as a dialogue; do not invent consent.

Алексей writes requests in plain Russian, by intent and symbol — not by file
...
1. Grep for every symbol Алексей names (function/class/method) to find ALL
...
4.5. If Алексей's request contains any URL, open that URL before writing the
...
4.6. Env/config/runtime boundary scan — REQUIRED whenever the request adds,
   ...
   if they exist. Defaults must be safe/off unless Алексей explicitly asked
   ...
   - return ``CLARIFICATION_NEEDED: ...`` if Алексей's request requires live
...
* Because ``created_by`` is ``zhvusha``, include non-empty
  ...
  - Prefer primary/direct sources. Weak/forum/social sources may motivate
    investigation, but a spec/proposal needs direct evidence unless Алексей
...
* Keep ``allowed_simplifications: []`` unless Алексей explicitly approved a
...
* Future self-growth / internet-parsed topics follow the same rule: prefer
  ...
  broad cleanup unless Алексей explicitly approved that simplification.
...
def build_user_prompt(
    *,
    ...
    ``request`` is Alexey's free-text. ``research_findings`` are produced
