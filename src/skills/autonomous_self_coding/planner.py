...
class AutonomousSelfCodingEngine:
    ...
    def _approve(
        ...
        *,
        ...
        mandate = (
            "Tier 3 autonomous mandate from Алексей; post-implementation "
    ...
    def _autonomous_blocker(self, spec: SpecModel) -> str:
        ...
        if spec.tier >= 3 and spec.approved_by != "nikita":
            return (
                "Tier 3 requires Алексей's explicit chat approval and discussion "
    ...
    *,
    ...
    return SelfImprovementCycleResult(
        ...
        summary=(
            ...
            "нужно свежее подтверждение Алексея, что это всё ещё надо делать."
            f"{blocker_note}"
        ),
        spec_slug=spec.slug,
        next_actions=(
            f"Спросить Алексея, актуальна ли `{spec.slug}`.",
...
def _build_ideation_request(
    ...
    *,
    ...
    return (
        ...
        "and must remain pending for Алексей's short chat approval; do not "
...
def _approval_next_actions(
    *,
    ...
    if spec.tier >= 3:
        return (
            (
                f"Ask Алексей briefly in chat: Это Tier 3 по `{spec.slug}`. "
                "Разрешаешь запуск или сначала обсудим правки?"
            ),
            "Do not paste the full spec unless Алексей asks for details.",
            "Classify Алексей's free-text decision through the AI approval classifier.",
        )
