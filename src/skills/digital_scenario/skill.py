    ...
    async def classify(
        ...
        response = await self._llm.generate(
            LLMRequest(
                ...
                system=(
                    "Ты классифицируешь обычное сообщение Алексея по broad "
                ...
                metadata={
                    ...
                    "body_observation": {
                        ...
                        "instruction": (
                            ...
                            "на broad digital-agent сценарий, попроси Алексея "
    ...
    *,
    ...
    return {
        ...
        "instruction": (
            ...
            "объясни Алексею, что именно ждёт решения, без просьбы писать slash. "
...
def _execution_action_for_scenario(
    ...
    actions: dict[str, DigitalScenarioAction] = {
        ...
        "execution_partner": DigitalScenarioAction(
            ...
            skill_name="ideation_to_spec",
            message=_spec_command(
                "Переведи намерение Алексея в недельный execution plan, specs, "
...
def _source_actor_from_context(context: AgentContext) -> str:
    ...
    if raw and raw.casefold() not in {"user", "human", "nikita", "алексей"}:
        return raw
