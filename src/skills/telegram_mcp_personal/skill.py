...
чате. Если адресат — Алексей, создатель, автор или "мне", используй chat_id
"creator".

Поле "message" — это точный текст, который будет показан Алексею на approval и
после подтверждения уйдёт во внешний Telegram. Не клади туда мета-инструкцию
вроде "что-нибудь", "от себя", "придумай сама". Если Алексей явно просит Жвушу
...
def _normalize_chat_id(value: object, default_chat_id: str = "") -> str:
    ...
    if chat_id.lower() in {"creator", "nikita", "алексей", "me", "self"}:
    ...
    *,
    ...
    if recent:
        blocks.append(
            ...
            "Если новое сообщение Алексея без явного другого адресата продолжает "
            "или исправляет прошлое Telegram-действие, используй "
            "previous_telegram_chat_id. Человеческое имя без @username или numeric id "
            "не является новым chat_id: если Алексей пишет только имя/падежную форму "
            "вроде «Тоше», «Тохе», «ему», а в текущей ветке уже есть "
            "previous_telegram_chat_id, используй previous_telegram_chat_id. Если "
            "Алексей явно назвал другого @username/id или явно попросил написать ему "
            "самому, используй новый адресат. Если Алексей исправляет адресата "
    ...
    if conversation:
        blocks.append(
            ...
            "команды. Если Алексей пишет короткое follow-up вроде "
    ...
    blocks.append(f"Новое сообщение Алексея:\n{message.strip()}")
    return "\n\n".join(blocks)
...
def _pending_reply_prompt(
    ...
    return (
        ...
        "Новый ответ Алексея на уточнение:\n"
        f"{message.strip()}\n\n"
        "Если Алексей дал буквальный текст для отправки — верни его как message. "
        ...
        "Если Алексей задаёт вопрос о формулировке Жвуши, исправляет её падеж/"
        "предлог или обсуждает само уточнение, а не даёт текст для отправки, "
        "верни action none с высокой confidence. Если Алексей исправляет адресата "
...
def _approval_revision_prompt(
    ...
    return (
        "Алексей смотрит pending approval для личного Telegram сообщения и "
        ...
        "плохой текст. Если Алексей явно отменяет действие или задаёт вопрос "
