...
Если Алексей просит обсудить идею, отвечай как собеседник-инженер: уточняй,
спорь с плохой идеей, показывай trade-off. Если нужен plan/spec, скажи что
готова оформить его только по явному сигналу Алексея: "оформи план".
    ...
    if state.recovery_kind is not None:
        ...
        if not state.recovery_needs_user_decision:
            return (
                ...
                "или архитектуры к Алексею. Дополнительные сообщения сохраню как "
...
def _dialogue_bootstrap_messages(context: AgentContext) -> tuple[str, ...]:
    ...
    for raw_line in recent.splitlines():
        ...
        if line.startswith("Собеседник:"):
            line = f"Алексей:{line[len('Собеседник:') :]}"
        if line.startswith(("Алексей:", "Жвуша:")):
    ...
    if dialogue_state.last_user_message:
        messages.append(f"Алексей: {dialogue_state.last_user_message}")
...
def _build_recovery_spec_create_text(state: ChatSelfCodingState, text: str) -> str:
    ...
    return (
        ...
        f"{recovery_error}\n\n"
        "Обсуждение после ошибки и новое решение Алексея:\n"
        f"{discussion}\n\n"
    ...
    *,
    ...
    if not needs_user_decision:
        return (
            ...
            "вопрос продукта или архитектуры к Алексею. Повтор сама не запускаю, "
...
class ChatSelfCodingSkill(InlineSkill):
    ...
    async def execute(self, message: str, context: AgentContext) -> SkillResult:
        ...
        state = state.append_message(f"Алексей: {text}")
        ...
        if state.recovery_kind is not None and not state.recovery_needs_user_decision:
            ...
            response = (
                "Это технический blocker, не вопрос выбора к Алексею. Я сохранила "
    ...
    async def _handle_goal(
        ...
        await self._state_store.save(
            updated.append_message(f"Алексей зафиксировал цель /код: {goal_text}")
        )
        return SkillResult(
            success=True,
    ...
    async def _run_implementation_goal(
        ...
        """Run implementation until it succeeds or needs Алексей's decision."""
            ...
            if not result.success or "dry-run" in result.response.lower():
                ...
                if (
                    ...
                    retry_note = (
                        ...
                        f"ошибке без вопроса к Алексею: {detail[:500]}"
                    )
                    retry_state = (
                        state.clear_recovery()
