...
_LLM_SYSTEM_TEMPLATE = (
    ...
    "create_spec ставь только когда Алексей явно просит оформить или пересобрать "
    "plan/spec: 'оформи план', 'пересобери план', 'создай spec'. "
    "approve ставь только на стадии pending_approval и только если Алексей явно "
...
def _build_llm_request(ctx: IntentClassifierContext) -> LLMRequest:
    approval_mode = (
        ...
        "словами как правилом. Смотри на смысл ответа: Алексей действительно "
