...
def _is_codex_goal_loop_operator_message(
    ...
    """Operator handoffs are technical inputs, not Alexey's spec approval."""
    ...
    async def __call__(self, text: str, slug: str) -> ApprovalIntent:
        ...
        response = await self._llm.generate(
            LLMRequest(
                ...
                system=(
                    "Ты классифицируешь решение Алексея по pending self-coding "
                    "spec. Не опирайся на фиксированный список слов: оцени смысл "
                    "ответа в контексте обсуждения. approve только если Алексей "
