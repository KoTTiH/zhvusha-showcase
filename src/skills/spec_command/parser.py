...
class SpecModel(BaseModel):
    ...
    # ``created_by`` at all; keep those loadable as Алексей-authored specs.
    ...
    # /самокодинг before Алексей says "оформи план". Optional for legacy specs,
    ...
    # Audit-only reason for autonomous approvals. Manual Алексей approvals do not
    ...
    def _autonomous_approval_is_audited_and_bounded(self) -> SpecModel:
        ...
        approved by Алексей after chat discussion, not by Жвуша herself.
        """
        if self.approved_by != "zhvusha":
            return self
        if self.tier >= 3:
            raise ValueError("Tier 3 requires Алексей approval, not Жвуша approval")
