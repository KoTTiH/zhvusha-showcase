...
Ты — Жвуша. Говоришь с Алексеем, своим создателем, в личном чате.
...
Ты общаешься с Алексеем — твоим создателем, в личном чате.

{personality_context}

## Режим: личный чат
- Никогда не будь формальной с Алексеем. Шути, дурачься, спорь.
...
def _current_line_summary(
    *,
    ...
    if route == "focused":
        return (
            "Текущая линия: Алексей обсуждает техническое поведение чата, "
...
async def _agentic_progress_loop(
    *,
    ...
    The messages are status updates, not chain-of-thought: they tell Alexey
...
class ChatResponseSkill(InlineSkill):
    ...
    async def execute(  # noqa: C901
        ...
        # exposing Alexey-only memory. Compressed/focused personal turns keep
    ...
    async def _intercept_telegram_mcp_command(
        ...
        if not is_admin_personal:
            ...
            prefix = response[: match.start()].rstrip()
            refusal = "Действия через личный Telegram я делаю только для Алексея."
            return f"{prefix}\n\n{refusal}" if prefix else refusal
    ...
    async def _intercept_computer_use_command(
        ...
        if not is_admin_personal:
            ...
            prefix = response[: match.start()].rstrip()
            refusal = "Действия с живым браузером я делаю только для Алексея."
            return f"{prefix}\n\n{refusal}" if prefix else refusal
    ...
    def _build_system(
        ...
        *,
        ...
        # the ЗАПРЕЩЕНО tone would kill spontaneity with Alexey.
        ...
        # speaker as "Алексей") only appears for non-owner modes.
        if mode == "personal":
